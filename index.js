...
client.on('messageCreate', (message) => {
	...
	lowerCasedMsg.match("dont") || lowerCasedMsg.match("n't"))) {
		positiveVibes = true;
		channel.send(`:smile: Thanks for mentioning ${amntOfNiceWords} positive ${plural(amntOfNiceWords)} around Anna! *${author}* **GAINS** ${amntOfNiceWords * 25} Anna Points. :smile:
		Message: **"${message.content}"** `);
	}

	/*Bad vibes case */
	if (amntOfBadWords > 0) {
		channel.send(`:anger: *${author}* said ${amntOfBadWords} bad ${plural(amntOfBadWords)} around Anna! *${author}* **LOSES** ${amntOfBadWords * 25} Anna Points. :anger:
		Message: **"${message.content}"** `);
	}

	/*Neutral vibes case */
	if (amntOfBadWords == 0 && amntOfNiceWords == 0) {
		channel.send(`${author} gets 10 Anna Points for talking about me.
		Message: **"${message.content}"**`);
		netPointChange = 10;
	}
	else {
